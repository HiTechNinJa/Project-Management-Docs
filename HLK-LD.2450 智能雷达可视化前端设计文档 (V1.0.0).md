# HLK-LD.2450 智能雷达可视化前端设计文档 (V1.0.0)
...
- **多目标追踪**：以不同颜色的圆点实时渲染最多 3 个目标（Target 1-3） 3,
  4。
...
实时流,/ws/radar/live?mac={id},WS,获取 10Hz 实时坐标推送 6

历史回溯,/api/v1/history/points,GET,\"参数：start_time,
end_time。获取历史轨迹点 12\"

守卫日志,/api/v1/guard/events,GET,获取入侵报警的历史列表与坐标节点 13

下发指令,/api/v1/device/command,POST,\"存储待执行指令（重启、切模式、设预警区）
14, 20\"

**比喻理解**：这份设计将前端从一个单纯的"**显示器**"升级为了一个"**智能指挥台**"。扇形图是你的**雷达站大屏幕**；"守卫模式"就像是给屏幕加了一层**红外感应报警框**，一旦有人越界就会在**日志本**（数据库）上记下一笔；而"历史回溯"则像是一台**时光机**，能让你在扇形图上重新看到几天前那个目标是如何移动的。
