# HLK-LD.2450 智能雷达可视化前端设计文档 (V1.0.3)
...
**前端实现**: 暂时使用设备列表API中的状态信息进行展示。

### 4.4 指令下发API (POST /api/v1/device/command)
...
- **API规划更新**：统一历史回溯API路径为/api/v1/radar/history，移除暂未实现的设备状态API。
...
- **API文档同步**：移除后端暂未实现的设备状态API (/api/v1/device/status)，避免前端开发 confusion。
- **文档一致性**：确保所有文档中的API列表与后端实际实现保持同步。
